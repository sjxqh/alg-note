...
vector<char>ans;
int tot;
pair<int, int> mp(int p, int q)
{
	pair<int, int>res = { p,q };
	auto& [x, y] = res;
	if (tot == 1)
	{
		swap(x, y);
...
signed main()
{
	...
	while (n > 1)
	{
		...
		if (op == 1)
		{
			...
			tot += r;
			tot %= 4;
			pair<int, int> p = mp(u, v),q=mp(x,y);
			u = min(p.first, q.first);
			x = max(p.first, q.first);
			v = min(p.second, q.second);
			y = max(p.second, q.second);
			rev(u, v, x, y, cnt);
		}
		else
		{
			...
			pair<int, int>p = mp(u, l), q = mp(d, r);
			u = min(p.first, q.first);
			d = max(p.first, q.first);
			l= min(p.second, q.second);
			r = max(p.second, q.second);
			if (tot & 1)o = -o;
			change(u, l, d, r, o);
		}
		n -= 6;
